...
今までの開発環境では、Gulpを用いてSCSSのコンパイルや圧縮、HTMLの構文チェックなどのシンプルな仕様でした。大規模な開発など関わることがないためです。
現在のCSS設計だと仕様変更の積み重ねによってカスケードが多重化され、CSSファイルからその状態を把握することが難しくなっていました。FLOCCSの場合でも役割が不明確なフォルダが増加するなど悪循環になってしまうからです。
